...
Task Deploy-packages {
    Exec { dotnet nuget push ./packages/Loment.$build_version.nupkg -s aza -k az }
}

Task Deploy-packages-release {
    Exec { dotnet nuget push ./packages/Loment.$build_version.nupkg  -s https://api.nuget.org/v3/index.json -k $NUGET_AUTH_TOKEN }
}
